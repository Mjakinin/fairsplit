...
export const FairSplitStore = {
  ...
  deleteAccount(accountId: string) {
    ...
    globalMembers = globalMembers.filter((m) => m.user_id !== accountId);
    if (globalCurrentUser?.id === accountId) {
      globalCurrentUser = null;
    }
    saveToStorage();
    notify();
  ...
  getFullDatabaseDump(): object {
    initStore();
    return {
      exported_at: new Date().toISOString(),
      system: 'FairSplit v2.2 Admin Backup',
      statistics: {
        total_accounts: globalAccounts.length,
        total_groups: globalGroups.length,
        total_expenses: globalExpenses.length,
        total_settlements: globalSettlements.length,
      },
